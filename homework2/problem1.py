...
'''
    Problem 1: Logistic Regression (with NumPy) (30 points)
...
#----------------------------------------------------
'''
    (Value Forward Function 2) Suppose we are given a logistic regression model and we have already computed the linear logit z on a data sample x (i.e., z(x)). Please compute the sigmoid activation on the data sample a(x). 
    ---- Inputs: --------
        * z: the logit value of the instance, a float scalar.
    ---- Outputs: --------
    ...
    ---- Hints: --------
        ...
        * When computing exp(-z), you need to be careful about an overflowing case. When the (-z) is a large number (say 1000),  the computer can no longer store the result in a floating-point number. In this case, we may want to avoid computing exp(-z) and assign the final result of a(x) directly.. 
        * This problem can be solved using 1 line(s) of code.
'''
#---------------------
def compute_a(z):
    ...
    '''  
        TEST: Now you can test the correctness of your code above by typing the following in the terminal:
        ---------------------------------------------------
        nosetests -v test1.py:test_compute_a
        --- OR ---- 
        python3 -m nose -v test1.py:test_compute_a
        --- OR ---- 
        python -m nose -v test1.py:test_compute_a
        ---------------------------------------------------
    '''
...
#----------------------------------------------------
'''
    (Local Gradient 2) Suppose we are given a logistic regression model and we have already computed the linear logit z(x) and activation a(x) on a training sample x. Please compute the gradient of the activation (a) w.r.t. the linear logit z. 
    ---- Inputs: --------
        * a: the activation value, a float scalar.
    ---- Outputs: --------
        * da_dz: .
...
def compute_da_dz(a):
    ...
    '''  
        TEST: Now you can test the correctness of your code above by typing the following in the terminal:
        ---------------------------------------------------
        nosetests -v test1.py:test_compute_da_dz
        --- OR ---- 
        python3 -m nose -v test1.py:test_compute_da_dz
        --- OR ---- 
        python -m nose -v test1.py:test_compute_da_dz
        ---------------------------------------------------
    '''
...
#----------------------------------------------------
'''
    (Using Logistic Regression 1) Given a trained logistic regression model with parameters w and b. Suppose we have a test sample x. Please use the logistic regression model to predict the label of x and the probability of the label of x being positive, i.e. the activation a(x). 
    ---- Inputs: --------
        * x: the feature vector of a data sample, a float numpy vector of length p.
        * w: the weights parameter of the logistic model, a float numpy vector of length p.
        * b: the bias value of the logistic model, a float scalar.
...
def inference(x, w, b):
    ...
    '''  
        TEST: Now you can test the correctness of your code above by typing the following in the terminal:
        ---------------------------------------------------
        nosetests -v test1.py:test_inference
        --- OR ---- 
        python3 -m nose -v test1.py:test_inference
        --- OR ---- 
        python -m nose -v test1.py:test_inference
        ---------------------------------------------------
    '''
...
#----------------------------------------------------
'''
    (Using Logistic Regression 2) Given a trained logistic regression model with parameters w and b. Suppose we have a test dataset Xtest (features). For each data sample x in Xtest, use the logistic regression model to predict the label of x. 
    ---- Inputs: --------
        * Xtest: .
        * w: the weights parameter of the logistic model, a float numpy vector of length p.
        * b: the bias value of the logistic model, a float scalar.
...
def predict(Xtest, w, b):
    ...
    Ytest = np.zeros(n_test) # initialize labels as all zeros
...
''' 
    TEST problem 1: 
        ...
        * (2 points) compute_dL_db ... ok
        * (2 points) compute_dL_dw ... ok
        * (1 points) backward ... ok
